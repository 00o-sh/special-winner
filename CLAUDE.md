...
|---------|----------|--------|--------|-------|
| `3226` | 3226 | `10.0.6.0/24` | Active | Default cluster, all tasks default to this |
| `usny01` | Rochelle | `10.1.6.0/24` | Prep | Infrastructure prepped, pending bootstrap |
| `wallenberg` | Wallenberg | TBD | Planned | Future cluster |
| `10th` | 10th | TBD | Planned | Future cluster |

The default cluster is `3226`. All tasks default to this cluster if no `CLUSTER` parameter is provided.

### DNS Architecture

Each cluster has unique subnets (routable over site-to-site VPN) to support future active-active-passive topologies.

**Per-cluster internal hostnames** (always resolve to that specific cluster's gateway):

| Record | Target | Purpose |
|--------|--------|---------|
| `internal-3226.00o.sh` | 3226's `CLUSTER_GATEWAY_ADDR` | Always reaches 3226 |
| `internal-usny01.00o.sh` | usny01's `CLUSTER_GATEWAY_ADDR` | Always reaches Rochelle |
| `internal-wallenberg.00o.sh` | wallenberg's `CLUSTER_GATEWAY_ADDR` | Always reaches Wallenberg |
| `internal-10th.00o.sh` | 10th's `CLUSTER_GATEWAY_ADDR` | Always reaches 10th |

These are created automatically by a DNSEndpoint in `kubernetes/apps/network/envoy-gateway/app/dnsendpoint.yaml` using `${CLUSTER_NAME}` substitution. Each cluster creates its own A record via unifi-dns.

**Shared hostnames** (follow workload placement):

| Record | Resolution Path | Follows Failover? |
|--------|----------------|-------------------|
| `external.00o.sh` | CNAME → Cloudflare tunnel | Yes (tunnel only runs on active clusters) |
| `internal.00o.sh` | A → local cluster's `CLUSTER_GATEWAY_ADDR` | Per-cluster (each cluster resolves to its own gateway) |
| `app.00o.sh` (external) | Cloudflare → tunnel → envoy-external | Yes (routes to cluster running the app) |
| `app.00o.sh` (internal) | k8s-gateway → envoy-internal | Local only (k8s-gateway only answers for locally running apps) |

**Internal access fallback**: When an app is suspended on the local cluster, k8s-gateway has no HTTPRoute for it → NXDOMAIN → Unifi DNS falls through to public DNS → Cloudflare tunnel → active cluster. LAN clients transparently reach workloads on remote active clusters via the external path.

**Active-active-passive** (future): Workloads can be split across multiple active clusters. Each cluster's k8s-gateway only resolves apps running locally. Cloudflare tunnel runs on all active clusters. Per-cluster internal hostnames provide direct access to any specific cluster regardless of workload placement.
...
- **2026-02-24**: Added per-cluster internal DNS endpoints (`internal-${CLUSTER_NAME}.00o.sh`) for direct cluster access, DNS architecture documentation for active-active-passive support
- **2026-02-24**: Updated cluster roster: 3226, Rochelle (usny01), Wallenberg, 10th
