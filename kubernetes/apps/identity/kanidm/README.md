...
These steps are required once after initial deployment:

```bash
# 1. Recover admin account
kubectl -n identity exec -it sts/kanidm -- kanidm recover-account admin

# 2. Recover idm_admin account (for day-to-day identity management)
kubectl -n identity exec -it sts/kanidm -- kanidm recover-account idm_admin

# 3. Verify server is running
kubectl -n identity exec -it sts/kanidm -- kanidm self whoami
```
...
```bash
kubectl -n identity exec -it sts/kanidm -- \
  kanidm system oauth2 show-basic-secret <client-name>
```
...
```bash
# List all persons
kubectl -n identity exec -it sts/kanidm -- kanidm person list

# List all groups
kubectl -n identity exec -it sts/kanidm -- kanidm group list

# List OAuth2 clients
kubectl -n identity exec -it sts/kanidm -- kanidm system oauth2 list
...
kubectl -n identity exec -it sts/kanidm -- \
  kanidm system oauth2 update-scope-map <client> <group> openid email profile groups
